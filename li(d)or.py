...
class Trial:
    def __init__(self,stimulus,params,win, success_count,failure_count,cue):
        ...
        # todo change param name in yml
        self.success_window = self.params['itis'] #this is the time window after pic diappear that still considered as success
    ...
    def change_success(self):
        if self.RT < (self.stim_time - pre_cue) + self.success_window:
            self.success = 1
        else:
            self.success = 0
    ...
    def run_trial(self):
        if self.stimulus.show:
            ...
            psychopy.core.wait(self.stim_time - pre_cue)  # wait for the remaining time after the cue until the stimulus disappears
